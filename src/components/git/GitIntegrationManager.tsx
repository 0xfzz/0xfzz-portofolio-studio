    ...
    <div className="space-y-10 pb-32 w-full">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1.5">
                <div className="flex items-center gap-3">
                  <span className={`text-[9px] font-mono font-bold ${repoUrl ? 'bg-[#777E65]' : 'bg-gray-400'} text-white px-2 py-0.5 uppercase rounded-[2px]`}>
                    {repoUrl ? 'CONNECTED' : 'DISCONNECTED'}
                  </span>
                </div>
                <div className="flex items-center gap-2 font-mono text-[13px] text-gray-900">
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
              <span className="text-[10px] font-mono font-bold uppercase text-gray-400">TARGET BRANCH</span>
      ...
      <div className="bg-[#fafafa] border border-gray-200 p-8 lg:p-10 space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-mono uppercase text-gray-400 pl-1">
      ...
      <div className="bg-[#fafafa] border border-[#cf222e] p-8 lg:p-10 space-y-8">
        <div className="flex items-center gap-3 text-[#cf222e]">
          ...
          <span className="text-[14px] font-sans font-semibold uppercase">DANGER ZONE</span>
        </div>
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1.5">
            <span className="text-[12px] font-sans font-bold text-gray-900 uppercase block">DESTRUCTIVE ACTIONS</span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
            <button 
              onClick={handleSync}
              disabled={syncing}
              className="w-full sm:w-auto px-6 py-3 border border-[#cf222e] text-[#cf222e] text-[10px] font-bold uppercase] hover:bg-red-50 transition-colors rounded-none bg-white text-center disabled:opacity-50"
            >
              {syncing ? 'SYNCING...' : 'FORCE PULL (OVERWRITE\nLOCAL)'}
            </button>
            <button 
              onClick={handleDisconnect}
              className="w-full sm:w-auto px-6 py-3 bg-[#cf222e] text-white text-[10px] font-bold uppercase] hover:bg-[#a52a2a] transition-colors rounded-none text-center"
            >
      ...
      <div className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-6 px-10 lg:px-12 z-30 flex justify-center transition-all duration-300 ${isDiffOpen ? 'blur-sm grayscale-[0.5] opacity-80 pointer-events-none' : ''}`}>
        <div className="w-full flex items-center justify-between pl-[240px]">
            ...
            <button 
              onClick={fetchConfig}
              className="w-[92px] h-[42px] bg-gray-50 border border-gray-800 text-gray-800 text-[12px] font-mono font-bold hover:bg-gray-100 transition-colors rounded-none"
            >
                RESET
            </button>
            <button 
              onClick={handleSave}
              disabled={saving}
              className="w-[207px] h-[42px] bg-gray-800 text-white text-[12px] font-mono font-bold hover:bg-black transition-colors rounded-none shadow-none flex items-center justify-center gap-3 disabled:opacity-50"
            >
